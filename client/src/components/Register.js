...
function Register(props) {
	...
	const handleRegisterSubmit = async (e) => {
		...
		if (firstName.length === 0) {
			...
			await axios.post('https://rowlogger.herokuapp.com/api/userrowsnew', userrows).then((res) => {
				console.log('successfully created user rows');
			});
			await axios.post(`https://rowlogger.herokuapp.com/api/users`, user).then((res) => {
